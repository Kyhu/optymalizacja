...
ns = size(Tau0,2); % liczba przelaczen

H = eye(n);
Tau_vec = reshape(Tau0,1,[]);
qi_value = qi_fun(0,Tau_vec,0,0); % Poczatkowa wartosc funkcji
...
while(STOP_CONDITION == 0)
    ...
    opt.gradd = d_vec*g_vec'; % rzut gradientu na kierunek poszukiwania (musi byc ujemny)
    opt.maxstep = max_step(Tau, d); % Maksymalny krok w poszukiwaniu liniowym
    %opt.maxstep = 100; % Maksymalny krok w poszukiwaniu liniowym
    Tau_vec = reshape(Tau,1,[]);
    
    [Tau_next_vec, qi_value] = line_search(qi_fun,Tau_vec,d_vec,opt,1);
    Tau_next = reshape(Tau_next_vec, [], ns);
